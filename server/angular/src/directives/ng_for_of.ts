...
export class NgForOfNb<T> implements DoCheck, OnChanges {
  ...
  ngDoCheck(): void {
    console.log(`in ngDoCheck server-common ngForOf.`);
    if (this._differ) {
  ...
  _applyChanges(changes: IterableChanges<T>) {
    console.log(`in ngForOf._applyChanges server-common.`);
    function asyncEach(items, fn, i?){
      if (items.length <= 0) return;
      if (i >= items.length) return;
      if (!i) i = 0;

      items.slice(i, i+50).forEach(function(item) {
        fn(item);
      });
    ...
    var currentIndex = 0;
    asyncEach((<any>changes).collection, (change) => {
      var item = {currentIndex: currentIndex, item: change, previousIndex: null, trackById: change};
      var view = this._viewContainer.createEmbeddedView(this._template, new NgForOfContext(((null)), this.ngForOf, -1, -1), currentIndex);
